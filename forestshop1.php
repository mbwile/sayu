...
<a href="login.php"class="btn btn-primary my-2" style="float:right;color;background-color:yellow;">Log out</a>
</h2>
<?php
// Check connection
if(isset($_POST['add'])){
...
$ombi = "INSERT INTO mbegu VALUES('','$users_id','$name','$quantity','$price','$now')";
mysqli_query($con, $ombi);
}

$ombi= "SELECT *FROM mbegu WHERE `users_id` = $users_id";
...
while($row = mysqli_fetch_array($result)){
  ...
  // echo "<h1>" .$row['name']. "</h1>";
   ...
   // echo" <a href='?isEditing=1&editId=$id'>Edit</a>
    ...
    <div class='post'>  
        ...
        <a href='editmbegu.php?id=$id'>Edit</a>
        <button onclick='futaPost($id)'>Delete</button>
    </div>
    </div>
    ";
 ...
 if(isset($_GET['isEditing']) && $_GET['isEditing']==1){
     $editId = $_GET['editId'];
     $isEditing = 1;
     $query = "SELECT * FROM mbegu WHERE id = '$editId'";
